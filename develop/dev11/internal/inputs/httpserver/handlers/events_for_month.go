...
// EventsForMonthHandler handler struct
type EventsForMonthHandler struct {
	service *app.App
}

// NewEventsForMonthHandler constructor
func NewEventsForMonthHandler(service *app.App) *EventsForMonthHandler {
	return &EventsForMonthHandler{service: service}
}

func (h *EventsForMonthHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) (any, *middlewares.ServeHTTPError) {
	...
	appResp, err := h.service.EventsForMonth(appReq)
	if err != nil {
		return nil, &middlewares.ServeHTTPError{
			InternalError: err,
